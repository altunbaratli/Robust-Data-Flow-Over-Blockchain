...
class fabricService{
   ...
   async enrollAdmin(adminName, password) {
    ...
    async getAllStates(username, channel, smartcontract, args) {
        try {
            ...
            // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
            // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
            const result = await contract.evaluateTransaction('getAllStates', args.startID, args.endID);
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
            return result.toString();
        } catch (error) {
            console.error(`Failed to evaluate transaction: ${error}`);
    ...
    async getState(username, channel, smartcontract, args) {
        try {
            ...
            const result = await contract.evaluateTransaction('getState', args.sensorID);
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
            return result.toString();
        } catch (error) {
            console.error(`Failed to evaluate transaction: ${error}`);
